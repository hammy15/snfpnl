...
export function ActionItems({ facilityId }: ActionItemsProps) {
  ...
  const { data, isLoading, error } = useQuery<ActionItemsResponse>({
    ...
    queryFn: async () => {
      const url = facilityId
        ? `http://localhost:3002/api/action-items?facilityId=${facilityId}`
        : 'http://localhost:3002/api/action-items';
  ...
  const addTaskMutation = useMutation({
    mutationFn: async (task: Partial<ActionItem>) => {
      const response = await fetch('http://localhost:3002/api/action-items', {
        method: 'POST',
  ...
  const updateTaskMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<ActionItem> }) => {
      const response = await fetch(`http://localhost:3002/api/action-items/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
