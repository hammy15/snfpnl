...
export function CustomKPIBuilder({ facilityId }: CustomKPIBuilderProps) {
  ...
  const { data: customKPIs = [], isLoading } = useQuery<CustomKPI[]>({
    ...
    queryFn: async () => {
      const response = await fetch('http://localhost:3002/api/custom-kpis');
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`http://localhost:3002/api/custom-kpis/${id}`, {
        method: 'DELETE'
      });
  ...
  const testFormula = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3002/api/custom-kpis/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
