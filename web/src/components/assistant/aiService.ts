...
import { getKPIValue, formatValue, calculatePeerRank, getTrendDirection } from './helpers';
import type {
  Facility,
  KPIData,
  AllKPIData,
...
function buildAIContext(
  ...
  const bottomPerformers = marginData
    .slice(-5)
    .reverse()
    .map(f => ({
      name: f.name || f.facility_id,
  ...
  const skilledMixData = allKPIs.filter(
    k => k.kpi_id === 'snf_skilled_mix_pct' && k.value !== null
  );
  const avgSkilledMix =
    skilledMixData.length > 0
      ? skilledMixData.reduce((sum, k) => sum + (k.value || 0), 0) / skilledMixData.length
      : null;

  const contractLaborData = allKPIs.filter(
    k => k.kpi_id === 'snf_contract_labor_pct_nursing' && k.value !== null
  );
  const avgContractLabor =
    contractLaborData.length > 0
...
export function generateFallbackResponse(
  ...
  const marginRank = selectedFacility
    ? calculatePeerRank(
        allKPIs,
        selectedFacility.facility_id,
        'snf_operating_margin_pct',
        selectedFacility.setting
      )
    : null;
  ...
  if (selectedFacility && (input.includes('review') || input.includes('full') || input.includes('complete'))) {
    return generateFacilityReviewResponse(selectedFacility, margin, skilledMix, revenuePPD, expensePPD, contractLabor, marginRank, trendInfo, marginTrends, allKPIs, period);
  }

  if (selectedFacility && (input.includes('trend') || input.includes('history') || input.includes('past'))) {
    return generateTrendResponse(selectedFacility, marginTrends, trendInfo);
  }

  if (selectedFacility && (input.includes('peer') || input.includes('compare'))) {
    return generatePeerComparisonResponse(selectedFacility, margin, allKPIs);
  }

  if (selectedFacility && (input.includes('suggest') || input.includes('improve') || input.includes('recommend'))) {
    return generateSuggestionsResponse(selectedFacility, margin, skilledMix, revenuePPD, expensePPD, contractLabor, allKPIs);
  }

  if (input.includes('email') || input.includes('draft')) {
    return generateEmailResponse(selectedFacility, margin, skilledMix, revenuePPD, contractLabor, trendInfo, marginTrends, allKPIs, period);
  }

  if (input.includes('benchmark') || input.includes('industry')) {
    return generateBenchmarkResponse(selectedFacility, margin, skilledMix, revenuePPD, expensePPD, contractLabor, marginRank, allKPIs, period);
  }

  if (input.includes('peer') || input.includes('compare') || (input.includes('worst') && input.includes('best'))) {
    return generatePeerAnalysisResponse();
  }

  if (input.includes('trend') || input.includes('analysis') || input.includes('opportunit')) {
    return generateTrendAnalysisResponse(period);
  }

  if (input.includes('attention') || input.includes('alert') || input.includes('concern')) {
    return generateAlertResponse(s, period);
  }

  if (input.includes('top') || input.includes('best') || input.includes('performer')) {
    return generateTopPerformersResponse(period);
  }

  if (input.includes('summary') || input.includes('overview')) {
    return generateSummaryResponse(period);
  }

  if (input.includes('margin') || input.includes('profitability')) {
    return generateMarginAnalysisResponse(s, period);
  }

  return generateDefaultResponse(userInput, s);
}

// Individual response generators
function generateFacilityReviewResponse(
  facility: Facility,
  margin: number | null,
  ...
  marginRank: { rank: number; total: number; percentile: number } | null,
  trendInfo: { direction: string; change: number },
  marginTrends: TrendData[],
  allKPIs: AllKPIData[],
  period: string
): string {
  const marginStatus = margin !== null ? (margin >= 8 ? 'strong' : margin >= 0 ? 'moderate' : 'concerning') : 'unknown';
  const trendEmoji = trendInfo.direction === 'improving' ? '\u{1F4C8}' : trendInfo.direction === 'declining' ? '\u{1F4C9}' : '\u27A1\uFE0F';

  const topPerformers = allKPIs
    .filter(k => k.kpi_id === 'snf_operating_margin_pct' && k.setting === facility.setting && k.value !== null)
    .sort((a, b) => (b.value || 0) - (a.value || 0))
    .slice(0, 3);

  return `**Full Performance Review: ${facility.name}**

**Location:** ${facility.state} | **Setting:** ${facility.setting}
...
| Metric | Value | Status |
|--------|-------|--------|
| EBITDAR Margin | ${formatValue(margin, 'percentage')} | ${marginStatus === 'strong' ? '\u2705 Strong' : marginStatus === 'moderate' ? '\u26A0\uFE0F Monitor' : '\u{1F534} Needs Attention'} |
...
| Contract Labor | ${formatValue(contractLabor, 'percentage')} | ${contractLabor !== null && contractLabor <= 10 ? '\u2705 Low' : '\u{1F534} High'} |

**PEER RANKING:**
${marginRank ? `Ranked #${marginRank.rank} of ${marginRank.total} ${facility.setting} buildings (${marginRank.percentile}th percentile)` : 'Insufficient data for ranking'}
...
**TOP ${facility.setting} PERFORMERS FOR COMPARISON:**
...
function generateTrendResponse(
  ...
  if (marginTrends.length === 0) {
    return `I don't have enough historical data for ${facility.name} yet. This might be a new building in the system or data is still being loaded.`;
  }
  ...
  return `**Performance Trends: ${facility.name}** ${trendEmoji}
...
function generatePeerComparisonResponse(
  facility: Facility,
  margin: number | null,
  allKPIs: AllKPIData[]
): string {
  ...
  const facilityIndex = peers.findIndex(p => p.facility_id === facility.facility_id);
  const topPerformers = peers.slice(0, 5);
  const bottomPerformers = peers.slice(-5).reverse();
  const statePeers = peers.filter(p => p.state === facility.state);

  return `**Peer Comparison: ${facility.name}**

**Current Performance:**
- EBITDAR Margin: ${formatValue(margin, 'percentage')}
- Ranking: #${facilityIndex + 1} of ${peers.length} ${facility.setting} buildings

**TOP 5 ${facility.setting} PERFORMERS:**
${topPerformers.map((p, i) => `${i + 1}. ${p.name} (${p.state}): ${formatValue(p.value, 'percentage')}${p.facility_id === facility.facility_id ? ' \u2B05\uFE0F YOU' : ''}`).join('\n')}

**BOTTOM 5 ${facility.setting} BUILDINGS:**
${bottomPerformers.map((p, i) => `${peers.length - 4 + i}. ${p.name} (${p.state}): ${formatValue(p.value, 'percentage')}${p.facility_id === facility.facility_id ? ' \u2B05\uFE0F YOU' : ''}`).join('\n')}

**${facility.state} PEERS:**
${statePeers.length > 0 ? statePeers.slice(0, 5).map((p, i) => `${i + 1}. ${p.name}: ${formatValue(p.value, 'percentage')}${p.facility_id === facility.facility_id ? ' \u2B05\uFE0F YOU' : ''}`).join('\n') : 'No other buildings in this state'}
...
function generateSuggestionsResponse(
  ...
  return `**Improvement Suggestions: ${facility.name}**
...
function generateBenchmarkResponse(
  selectedFacility: Facility | null,
  margin: number | null,
  skilledMix: number | null,
  revenuePPD: number | null,
  ...
  if (selectedFacility && margin !== null) {
    const gaps: string[] = [];
    if (margin < 8) gaps.push(`- Margin: ${formatValue(8 - margin, 'percentage')} below target`);
...
|--------|-----------|-----------|--------|
| EBITDAR Margin | ${formatValue(margin, 'percentage')} | 8.0% | ${margin >= 8 ? '\u2705 Above' : margin >= 5 ? '\u26A0\uFE0F Near' : '\u{1F534} Below'} |
| Skilled Mix | ${formatValue(skilledMix, 'percentage')} | 20.0% | ${skilledMix !== null && skilledMix >= 20 ? '\u2705 Above' : skilledMix !== null && skilledMix >= 15 ? '\u26A0\uFE0F Near' : '\u{1F534} Below'} |
| Revenue PPD | ${formatValue(revenuePPD, 'currency')} | $400 | ${revenuePPD !== null && revenuePPD >= 400 ? '\u2705 Above' : revenuePPD !== null && revenuePPD >= 360 ? '\u26A0\uFE0F Near' : '\u{1F534} Below'} |
| Expense PPD | ${formatValue(expensePPD, 'currency')} | $350 | ${expensePPD !== null && expensePPD <= 350 ? '\u2705 Below (Good)' : expensePPD !== null && expensePPD <= 380 ? '\u26A0\uFE0F Near' : '\u{1F534} Above'} |
| Contract Labor | ${formatValue(contractLabor, 'percentage')} | 10.0% | ${contractLabor !== null && contractLabor <= 10 ? '\u2705 Below (Good)' : contractLabor !== null && contractLabor <= 15 ? '\u26A0\uFE0F Near' : '\u{1F534} Above'} |
...
function generatePeerAnalysisResponse(): string {
...
function generateTrendAnalysisResponse(period: string): string {
...
function generateAlertResponse(s: BotSettings, period: string): string {
...
function generateTopPerformersResponse(period: string): string {
...
function generateSummaryResponse(period: string): string {
...
function generateMarginAnalysisResponse(s: BotSettings, period: string): string {
...
function generateDefaultResponse(userInput: string, s: BotSettings): string {
