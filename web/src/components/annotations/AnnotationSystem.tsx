...
export function AnnotationSystem({ facilityId, periodId, kpiId }: AnnotationSystemProps) {
  ...
  const { data: annotations = [], isLoading } = useQuery<Annotation[]>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`http://localhost:3002/api/annotations?${params}`);
  ...
  const createMutation = useMutation({
    mutationFn: async (annotation: Partial<Annotation>) => {
      const response = await fetch('http://localhost:3002/api/annotations', {
        method: 'POST',
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`http://localhost:3002/api/annotations/${id}`, {
        method: 'DELETE'
      });
